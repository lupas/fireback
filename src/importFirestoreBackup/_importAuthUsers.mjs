...
export default async function({ projectId }) {
  ...
  try {
    ...
    const cliCmd = `firebase use ${projectId} && firebase auth:import ./${projectId}/AllAuthUsers.json --hash-algo=scrypt --rounds=8 --mem-cost=14 --hash-key=${answers.hashKey} --salt-separator=${answers.saltSeparator}`;
